...
def parse_arguments():
    ...
    parser.add_argument('--data_path', type=str, default='FailedESConv.json')
    ...
    parser.add_argument('--prompt_path', type=str, default='data/prompts/Error_Type_Statis.txt')
...
def process_single_dialog(dialogs, prompt, sys_prompt, args, api_key):
    ...
    for dialog in dialogs:
        strategy = ""
        if dialog["speaker"] == "speaker":
            user_input = _norm(dialog["content"])
        else:
            strategy = dialog["annotation"]["strategy"]
            response_text = _norm(dialog["content"])

            context_1 = context.replace(f"seeker: {user_input}\n", "")
        ...
        # Build context
        if dialog["speaker"] == "speaker":
            context += f"seeker: {user_input}\n"
        else:
            context += f"supporter: [{strategy}] {response_text}\n"
