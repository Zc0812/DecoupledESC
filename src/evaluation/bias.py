...
def strg_evaluation(data, save_path):
    strategy_order={} 
    ...
    for idx, item in enumerate(data):
        # total data
        gth=item['GT_strategy'].lower()
        pred=item['predict_strategy'].lower()
        ...
        # each strg data
        pred_strg=item['predict_strategy'].lower()
        if pred_strg in strg_list:
            pred_data[pred_strg]+=1
            strg_data[item['GT_strategy'].lower()].append(pred_strg)
        else:
            pred_data["others"]+=1
            strg_data[item['GT_strategy'].lower()].append("others")
    ...
    metrics['Macro F1 (Q)'] = macro_f1
    metrics = {k: float(v) for k, v in metrics.items()}

    results = {
        "total_metrics": metrics,
        "strategy_metrics": strg_metric
    }
    if save_path:
...
def checking_error(data, mode=None):
    ...
    for i, d in enumerate(data):
        if mode=="with_gold":
            ...
            if d['predict_strategy'].lower() in strg_list:
                total_data.append(d) 
            else:
                error_data.append(d)
                print(i, d['predict_strategy'])
    
    return total_data, error_data

def evaluate_strategy(data, save_path):
    total_data, error_data = checking_error(data)
    print(f"len_error_data: {len(error_data)} \nerror_ratio: {len(error_data) / len(data) * 100:.2f}%")
    
    strg_evaluation(total_data, save_path)
    


if __name__=="__main__":
    file_name = "ESC_qwen2.5-7B-sdpo-sft-lora.json"
